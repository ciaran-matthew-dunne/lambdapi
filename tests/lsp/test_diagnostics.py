...
class TestFocusedHints(LSPTestCase):
    """Keyword-anchored OK hints (#1444): hint ranges underline the
    command/tactic keyword — never the modifier, never the name, never
    the whole statement. Expected failures below flip to "unexpected
    success" once #1444 is merged and this branch rebases: remove the
    decorators then."""
    ...
    @unittest.expectedFailure  # pending #1444
    def test_symbol_declaration_hint_covers_keyword(self):
        """For `constant symbol Nat : TYPE;`, hint should underline
        `symbol` — the command keyword, not the modifier or the name."""
        uri, text, src, diags = self.open_fixture("simple.lp")
        nat_line, kw_col = src.find(r"constant symbol Nat", "symbol")
        hints = self.hints(diags)
        ...
        r = matching[0]["range"]
        self.assertEqual(r["start"]["character"], kw_col,
            f"hint should start at `symbol` (col {kw_col}), "
            f"got col {r['start']['character']}")
        self.assertEqual(
            r["end"]["character"] - r["start"]["character"], len("symbol"),
            "hint should span exactly the command keyword")
