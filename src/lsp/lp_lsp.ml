...
let do_completion ofmt ~id params =
  ...
  | Some doc ->
    ...
    | Some ss ->
      ...
      (* "In a proof" means the cursor's command carries a proof
         script. Every [P_symbol] node has a goals snapshot (symbol
         elaboration goes through proof machinery), so a non-empty
         [goals] field would not discriminate. *)
      let in_proof =
        match get_node_at_pos doc line character with
        | Some n ->
          (match Pure.Command.get_elt n.Lp_doc.ast with
           | P_symbol {p_sym_prf = Some _; _} -> true
           | _ -> false)
        | None -> false
      in
      let items = ref [] in
